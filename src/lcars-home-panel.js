...
.shell { --bg:#06070b; --apricot:#eab18c; --salmon:#e48878; --lilac:#baadd8; --sky:#83bdd2; --gold:#d7bd67; --ink:#101117; --muted:#b8b0bc; --mint:#8ed0a6; background:var(--bg); display:flex; flex-direction:column; min-height:100vh; padding:10px 10px 0; overflow:clip; color:#f6f0ea; }
...
.masthead { display:flex; align-items:center; justify-content:space-between; gap:14px; background:var(--apricot); border-radius:0 34px 0 0; padding:10px 22px; min-height:72px; } .masthead-copy { min-width:0; } .masthead small { display:block; color:rgba(10,10,14,.55); font-size:10px; font-weight:900; letter-spacing:.2em; } .masthead h1 { margin:2px 0 0; color:#101117; font-size:23px; letter-spacing:.1em; line-height:1.05; } .masthead time { color:rgba(10,10,14,.72); font-size:13.5px; font-weight:950; letter-spacing:.08em; white-space:nowrap; }
...
.security-panel { --panel:var(--sky); } .security-list { display:grid; grid-template-columns:1fr 1fr; padding:8px; gap:5px; } .security-row { background:#181a22; padding:7px 9px; display:flex; justify-content:space-between; gap:4px; align-items:center; color:#d6e0e6; font-size:10.5px; font-weight:800; letter-spacing:.05em; } .security-row:last-child { grid-column:span 2; } .security-row b { color:var(--sky); font-size:9.5px; } .security-row.alert { background:#412227; color:#ffd6cd; } .security-row.alert b { color:#ff9c8d; }
.cameras-panel { --panel:var(--sky); } .cameras { display:grid; grid-template-columns:1fr 1fr; gap:5px; padding:7px; } .camera { background:#15171e; min-width:0; min-height:0; aspect-ratio:16/9; overflow:hidden; position:relative; } .camera-stream { display:block; width:100%; height:100%; background:#0a0b0e; } .camera-frame { position:absolute; inset:0; display:grid; place-items:center; background:#0c0d11; } .camera-still { position:absolute; inset:0; width:100%; height:100%; object-fit:cover; opacity:.4; filter:grayscale(.4); } .camera-glyph { font-size:28px; color:rgba(234,177,140,.5); position:relative; } .camera-label { position:absolute; inset:auto 0 0; min-height:25px; padding:6px 7px; background:rgba(5,6,9,.78); display:flex; justify-content:space-between; align-items:center; color:#edf5f6; font-size:9.5px; font-weight:850; letter-spacing:.06em; pointer-events:none; } .camera-label b { color:var(--sky); font-size:9px; } .camera-offline .camera-label b { color:#ff9c8d; }
.events { padding:7px 9px; display:flex; flex-direction:column; gap:5px; } .event { display:grid; grid-template-columns:48px 1fr; gap:8px; align-items:start; border-left:3px solid var(--apricot); padding:6px 7px; background:#19171b; color:#f7dfd3; font-size:11.5px; line-height:1.3; } .event time { color:var(--apricot); font-weight:900; font-size:9.5px; letter-spacing:.04em; padding-top:1px; } .event.past { opacity:.57; border-left-color:#75615a; } .event.empty { display:block; background:transparent; border-left-color:transparent; color:#8d857c; font-size:11px; padding:2px 6px; }
.climate-panel { --panel:var(--salmon); } .climate-body { padding:10px 11px 9px; display:grid; grid-template-columns:1.1fr 1fr; grid-template-areas:"hero outside" "controls controls"; column-gap:12px; row-gap:9px; } .climate-hero { grid-area:hero; } .climate-hero small, .climate-outside small { display:block; color:var(--muted); font-size:9px; letter-spacing:.16em; font-weight:900; margin-bottom:4px; } .climate-hero strong { display:block; color:#ffe0d4; font-size:42px; line-height:.95; letter-spacing:-.04em; } .climate-hero em { display:block; font-style:normal; color:#e6a295; font-size:9px; font-weight:900; letter-spacing:.2em; margin-top:5px; } .climate-outside { grid-area:outside; align-self:center; padding-left:12px; border-left:1px solid rgba(244,136,120,.28); } .climate-outside b { display:inline-block; color:#f2d6cc; font-size:22px; margin-right:6px; line-height:1; } .climate-outside span { color:#c9b2b8; font-size:10px; font-weight:750; letter-spacing:.05em; text-transform:capitalize; } .climate-controls { grid-area:controls; display:grid; grid-template-columns:46px 1fr 46px; gap:6px; align-items:center; } .climate-setpoint { text-align:center; color:#e9c3b8; font-size:11px; font-weight:900; letter-spacing:.14em; background:#241a1d; border-radius:0 0 11px 0; padding:8px 0; } .adjust { height:34px; border:0; padding:0; display:flex; align-items:center; justify-content:center; background:var(--salmon); color:#141014; font-size:26px; line-height:1; font-weight:700; cursor:pointer; } .adjust:first-child { border-radius:0 0 0 11px; } .adjust:last-child { border-radius:0 11px 0 0; } .adjust:disabled { background:#5c454b; color:#8c7f83; cursor:not-allowed; } .service-error { background:#4a2228; color:#ffd4cc; font-size:10px; padding:5px 10px; }
.conditions-panel,.forecast-panel { --panel:var(--lilac); } .conditions { padding:9px 10px; min-height:62px; align-items:center; display:grid; grid-template-columns:repeat(3,1fr); gap:8px; } .conditions b { color:#e9e1fc; font-size:13px; } .conditions small { display:block; margin-top:3px; color:#aba2c1; letter-spacing:.05em; font-size:8.5px; font-weight:900; } .forecast-panel { display:flex; flex-direction:column; } .forecast { padding:7px 9px; display:grid; grid-template-columns:repeat(5,1fr); gap:5px; } .daily .forecast { grid-template-columns:repeat(4,1fr); } .forecast-item { min-width:0; min-height:62px; display:flex; flex-direction:column; align-items:center; justify-content:center; gap:1px; background:#1b1923; color:#e8e1f8; padding:4px 2px; } .forecast-item span,.forecast-item small,.forecast-item em { color:#a79ec2; font-size:9px; font-weight:850; letter-spacing:.04em; } .forecast-item span { font-size:9.5px; } .forecast-item em { color:#b9cde7; font-style:normal; } .forecast-item i { color:#c8b6e6; font-style:normal; font-size:21px; line-height:1; } .forecast-item b { font-size:15px; } .forecast-empty { grid-column:1 / -1; color:#8d857c; font-size:11px; letter-spacing:.04em; text-align:center; padding:4px 6px; }
.lights-panel { --panel:var(--mint); } .lights { display:grid; grid-template-columns:repeat(auto-fill,minmax(148px,1fr)); gap:5px; padding:8px; } .light-chip { min-width:0; display:flex; align-items:center; gap:8px; background:#181d19; padding:7px 9px; } .light-chip b { flex:none; width:8px; height:20px; background:var(--mint); } .light-chip span { min-width:0; color:#e6f0e8; font-size:11px; font-weight:800; letter-spacing:.02em; overflow-wrap:anywhere; } .lights-empty { color:#8d857c; font-size:11px; letter-spacing:.04em; padding:2px 6px; }
.feed-panel { --panel:var(--gold); } .feed { padding:9px 11px; color:#eae1d2; font-family:"Arial Narrow","Roboto Condensed","Helvetica Neue Condensed",sans-serif; font-stretch:condensed; font-size:13.5px; font-weight:600; line-height:1.5; letter-spacing:.03em; text-transform:none; } .feed p { margin:0 0 3px; } .feed p:last-child { margin-bottom:0; } .feed-panel.word .feed { font-weight:560; }
.footer { display:flex; align-items:center; justify-content:space-between; gap:10px; background:var(--apricot); min-height:46px; padding:0 20px; color:rgba(10,10,14,.68); font-size:11px; font-weight:950; letter-spacing:.18em; } .footer .footer-code { color:rgba(10,10,14,.88); }
